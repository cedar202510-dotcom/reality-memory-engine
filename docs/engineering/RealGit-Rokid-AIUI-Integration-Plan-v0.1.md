...
| `APP_INBOX` | 手机或 Web App 的消息与历史记录 |
...
1. 保持 PR #3 已实现的采集、下发和回执链路不变。
2. 创建最小 AIUI 对话智能体并打包 `.aix`。
3. 为 Agent Gateway 增加 `source` 与 `response_channel`。
4. AIUI 通过 HTTPS 调用后端；发布前在 Rokid 开发者后台登记域名。
5. 真机验证“乐奇 -> RealGit -> 后端记忆查询 -> AIUI 回答”。
6. 再验证 AIUI 对话期间六轴持续、媒体采集延后和会话结束恢复。
