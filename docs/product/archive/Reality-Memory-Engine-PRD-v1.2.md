...
首个可用产品以家庭为可信运行域，以“找物”为英雄场景，同时跑通耗材余量、口头偏好与任务、隐私暂停与遗忘。当前用 CXR-L 手机 App 验证 Rokid 图片与音频链路，最终产品使用运行在眼镜本机的原生 Glass App 完成低打扰采集；长期通过统一 Device Adapter 与 Source Contract 支持其他眼镜、手机、戒指、机器人和家庭设备。
...
1. 先通过 CXR-L iOS/Android 过渡探针取得真实眼镜图片和短音频，验证授权、连接、媒体质量与统一来源契约。
2. 获得裸机调试条件后，将同一采集和审计逻辑迁入 RV101 原生 Glass App，并验证佩戴/摘下、后台生命周期、无业务预览采集和本地 VAD。
...
Rokid 原生 Glass App 是最终采集客户端，不承载记忆本体。CXR-L 探针只承担开发线到位前的真实媒体验证，不作为最终后台感知方案。会话、策略、审计、证据协议和离线队列沉淀为通用 Edge Core；CameraX、AudioRecord、佩戴广播和按键实现位于 Rokid Adapter。
...
```text
┌──────────────────────────── 设备与交互层 ────────────────────────────┐
│ Rokid Native App │ CXR-L Probe │ Ring │ Phone │ Future Adapters   │
└──────────────────────────────────┬───────────────────────────────────┘
                                   │ Unified Source Contract
┌──────────────────────────── Edge Core ───────────────────────────────┐
│ Source Adapter │ Session FSM │ Local Policy │ Capture/VAD Scheduler│
│ Encrypted Buffer │ Upload/Relay │ Device Audit │ Secure Erase       │
└──────────────────────────────────┬───────────────────────────────────┘
                                   │ Ingest / Control API
┌──────────────────────────── Memory Platform ─────────────────────────┐
...
- 图片/短视频采集调度、本地音频 VAD、预算、节流和离线队列。
- 证据加密、过期与安全删除。
- 将厂商事件映射为统一 `SourceEnvelope`，并上报审计。
...
apps/
  ...
  cxr-l-probe/             # 开发阶段真实图片/音频过渡探针
  mobile-trust/            # 可选手机管理、配对、策略与审计
services/
  gateway/
  policy/
  evidence/
...
| CXR-L 过渡探针 | 当前 iOS，必要时 Android | 开发线到位前验证眼镜图片与音频 |
| 手机管理端 | 平台按产品需求选择 | 可选的配对、策略、审计、提醒和戒指桥接入口 |
...
| CXR-L SDK 1.0.4 | Android/iOS 手机 | 经 Rokid AI App 完成授权与连接，调用拍照、音频、自定义 View/App | 当前真实媒体过渡测试路径 |
...
- 开发线到位前，CXR-L 探针可经 Rokid AI App 获取拍照/音频用于解析链路测试。
...
1. **眼镜直连**：RV101 原生 Glass App 通过 HTTPS 获取临时上传授权并上传加密 Evidence；这是最终产品主通道。
2. **CXR-L 过渡测试**：开发线到位前，由手机端通过 CXR-L 获取受控图片或音频，再使用相同 `SourceEnvelope` 进入测试流水线；它不改变后端契约，也不承担最终后台感知。
3. **离线暂存**：两条通道都不可用时，仅在有效策略与 TTL 内保存加密证据；过期、暂停、摘下或删除请求到达后优先清理，不因离线而延长留存。

手机不是眼镜采集或 Memory Platform 的必要运行依赖。若首发戒指协议需要手机 BLE Gateway，或产品需要移动端管理、提醒和审计入口，可部署手机 App；没有手机 App 时，眼镜直连与中台链路仍应独立工作。当前实现不得依赖其他 Rokid 产品线的 P2P 或文件 API。
...
手机 App 是可选的配对、信任、管理和提醒入口，不是眼镜采集、结构化处理或 Memory Platform 的运行依赖。首发若部署手机端，核心页面包括：
...
首个 PoC 使用内部 Web 控制台即可替代手机 UI。后续是否建设手机 App 由戒指 BLE Gateway、用户管理体验和提醒渠道需求共同决定。
...
1. **Reality Edge**：运行在眼镜、可选手机端、戒指桥接器和未来设备上的来源适配、会话、策略、采集与证据层。
...
当前 CXR-L 探针是 Reality Edge 的过渡测试入口，Rokid 原生 Glass App 是首个最终设备实现。两者输出同一 Source Contract，后续硬件只增加 Adapter 和必要的能力描述，不重建感知、记忆和提醒流水线。产品先通过真机图片与短音频 PoC 证明设备能力和信任链，再逐步接入结构化观察、记忆主线和提醒服务，最终形成可被多硬件复用的现实世界记忆基础设施。
