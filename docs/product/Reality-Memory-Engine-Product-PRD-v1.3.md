...
音频分为显式语音和会话内 VAD 两种模式。显式语音优先级最高；会话内 VAD 只在用户已授权现实记录且音频策略允许时运行。静音缓冲不落盘、不上传、不进入 ASR。CXR-L 阶段因为没有独立 VAD 事件，只能先开启眼镜到手机的音频流，再在手机本地做 VAD。最终原生 Glass App 必须把 AudioRecord + VAD 移到眼镜端。
...
| 阶段 | 目标 |
|---|---|
| Phase 0 | Rokid、统一手机网关与戒指能力验证，确认 CXR-L 过渡能力、RV101 原生相机、音频、IMU、佩戴事件和手机加密队列 |
| Phase 1 | Rokid 定时截图 PoC，跑通佩戴会话、暂停、采集审计和图片删除 |
| Phase 2 | Edge Core 与隐私链，完成策略快照、证据缓冲、TTL、离线队列和删除回执 |
