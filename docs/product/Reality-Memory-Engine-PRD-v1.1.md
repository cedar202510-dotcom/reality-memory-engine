...
> 当前首发硬件：Rokid Glasses（RV101）、语音智能戒指、手机管理端  
...
首个可用产品以家庭为可信运行域，以“找物”为英雄场景，同时跑通耗材余量、口头偏好与任务、隐私暂停与遗忘。当前通过 Rokid Glass App 和语音戒指完成首批采集接入；长期通过统一 Device Adapter 契约支持其他眼镜、手机、机器人和家庭设备。
...
- 建立 `Evidence → ObservationCandidate → MemoryEvent → StateProjection` 的事实链。
...
1. Glass App 能接收佩戴/摘下事件并建立受控佩戴会话。
2. 用户收到一次“5 秒后开启现实记录”的轻提示，可在倒计时内关闭。
3. 会话激活后可以按固定间隔调用 CameraX 拍照，暂停或摘下后立即停止。
4. PoC 期间图片仅在设备本地完成采集验证并删除，服务端只接收无媒体审计元数据。
5. 采集基础可靠后，再接入 AI 分析、结构化记忆与家庭场景闭环。
...
视觉、语音或传感器模型只能创建 `ObservationCandidate`。候选经过策略、实体归因、置信度、冲突检测和交叉验证后，才生成追加式 `MemoryEvent`。当前状态由事件流计算，任何纠正都通过新事件完成。
...
Rokid Glass App 是首个采集客户端，不承载记忆本体。会话、策略、审计、证据协议和离线队列沉淀为通用 Edge Core；CameraX、AudioRecord、佩戴广播和按键实现位于 Rokid Adapter。
...
```text
┌──────────────────────────── 设备与交互层 ────────────────────────────┐
│ Rokid Glass App │ Ring Bridge │ Mobile Trust App │ Future Adapters │
└──────────────────────────────────┬───────────────────────────────────┘
                                   │ Unified Edge Contract
┌──────────────────────────── Edge Core ───────────────────────────────┐
│ Session FSM │ Local Policy │ Capture Scheduler │ Encrypted Buffer  │
│ Upload/Relay │ Device Audit │ Offline Queue │ Secure Erase         │
└──────────────────────────────────┬───────────────────────────────────┘
                                   │ Ingest / Control API
┌──────────────────────────── Memory Platform ─────────────────────────┐
│ Device Registry │ Policy Service │ Evidence Service │ AI Pipeline   │
...
- `ObservationCandidate` 是待决断言，不等同于已确认记忆。
...
```text
apps/
  rokid-glass/             # 眼镜本机 Android App
  mobile-trust/            # 手机管理、配对、策略与审计
services/
  gateway/
  policy/
  evidence/
  ...
  privacy/
packages/
  edge-contract/           # 会话、策略、采集与审计 schema
  memory-contract/         # Entity、Candidate、Event、Projection schema
  api-client-kotlin/
  api-client-python/
adapters/
  rokid-native/
...
| 层 | 技术基线 | 说明 |
|---|---|---|
| Rokid Glass App | Kotlin、Android API 31、Compose、CameraX、AudioRecord、SensorManager、Room | 与官方裸机开发路径一致 |
| 本地调度 | Foreground Service、协程、Room 顺序日志 | 真机验证后台生命周期 |
| 手机端 | Kotlin/Android 首发，后续 iOS | BLE 戒指桥接、策略、审计 |
...
- 主路径：在 RV101 眼镜本机运行 YodaOS-Sprite 裸机 Android Glass App。
- 设备能力：使用 CameraX、AudioRecord、SensorManager、佩戴/摘下广播与系统按键。
- 手机协同：CXR-L 仅用于授权、连接、能力联调或媒体中继的备选实现。
...
| 路径 | 运行位置 | 能力 | 在本项目中的角色 |
|---|---|---|---|
| 眼镜端裸机开发 | RV101 眼镜本机 | Android 12 / API 31、CameraX、AudioRecord、SensorManager、佩戴/摘下广播、按键 | 当前 Glass App 主路径 |
| CXR-L SDK 1.0.4 | Android/iOS 手机 | 经 Rokid AI App 完成授权与连接，调用拍照、音频、自定义 View/App | 当前手机协同备选路径 |
...
- 手机伴侣若需要直接经 Rokid AI App 获取拍照/音频，可接入 CXR-L。
...
| `RokidAudioAdapter` | 主动语音的 AudioRecord 采集 |
...
| `TransportAdapter` | 眼镜直连 HTTP 或手机中继 |
...
3. 若第三方应用无法在预期生命周期持续调用相机，切换为 CXR-L 手机协同；更深的厂商集成作为后续独立适配项。

真机 PoC 必须记录前台、熄屏、摘下、折叠、进程被杀、断网、锁屏和重启后的实际行为。

### 8.6 RV101 数据通道

眼镜端与平台之间使用统一 `CaptureEnvelope`，至少包含 `device_id`、`session_id`、`captured_at`、`monotonic_offset_ms`、`policy_snapshot_id`、`trigger`、`modality`、`idempotency_key` 和媒体密文引用。

当前按以下顺序实现和验证：

1. **眼镜直连**：RV101 Glass App 通过 HTTPS 获取临时上传授权并上传加密 Evidence；这是网络、后台和功耗验证通过后的首选通道。
2. **CXR-L 手机中继**：当 RV101 的直连网络或后台生命周期不能满足要求时，由手机端通过 CXR-L 获取受控媒体或设备事件，再使用相同 `CaptureEnvelope` 上传。
3. **离线暂存**：两条通道都不可用时，仅在有效策略与 TTL 内保存加密证据；过期、暂停、摘下或删除请求到达后优先清理，不因离线而延长留存。

手机管理端始终承担戒指 BLE Gateway、策略与审计入口，但是否承担眼镜媒体中继由 Phase 0 真机结果决定。当前实现不得依赖其他 Rokid 产品线的 P2P 或文件 API。
...
| 2–3 秒短视频 | 眼镜 | 连续动作、倒出、快速移动 | 静态帧歧义时使用 |
| 主动语音 | 戒指/眼镜/手机 | 偏好、任务、命名、纠错 | 用户显式触发 |
...
| 环境 | 媒体 | 默认策略 |
|---|---|---|
| Rokid 采集 PoC | 截图 | 成功验证后立即删除，零外传 |
| 正式处理链 | 图片 | 本地最长 10 分钟；云端最长 15 分钟；完成结构化即删 |
| 正式处理链 | 短视频/主动音频 | 本地与云端最长 15 分钟；完成结构化即删 |
...
| `SourceEvent` | 设备或用户入口产生、尚未经过模型解释的标准化输入 |
| `Evidence` | 短暂媒体或传感器证据 |
| `ObservationCandidate` | 尚未成为事实的模型断言 |
| `MemoryEvent` | 追加式、可追溯的事实变化 |
| `StateProjection` | 从事件流计算的当前最佳状态 |
| `Signal` | 对 Agent 有价值的状态变化通知 |
...
| `ingested_at` | 平台成功接收 `SourceEvent` 或 Evidence 元数据的时间 |
...
| SourceEvent / Candidate | PostgreSQL | 按处理与审计策略 | 否 |
...
source_events(
  id, household_id, actor_id, device_id, session_id,
  source_type, occurred_at, observed_at, ingested_at,
  payload_json, policy_snapshot_id, idempotency_key
)

evidence(
  id, household_id, device_id, session_id, modality, trigger,
  captured_at, policy_snapshot_id, storage_ref, key_id,
  ttl_until, retention_state, deleted_at, metadata_json
)

observation_candidates(
  id, household_id, evidence_id, candidate_type, entity_id,
  assertion_json, confidence_json, status, conflict_set_id,
  model_name, model_version, created_at, resolved_at
)
...
- `observation_candidates(status, created_at)`
- `evidence(ttl_until, retention_state)`
- `source_events(household_id, observed_at desc)`
- `capture_attempts(device_id, session_id, scheduled_at)`
- `places(household_id, parent_id)`
- `signals(household_id, status, priority, created_at)`
...
- 按模态选择 Vision、ASR、视频或传感器 Extractor。
- 控制重试、超时、成本和模型版本。
- 将模型结果写入候选存储。
...
| 主动语音 ASR | 可私有化中文 ASR | 短句准确率、端到端时延 |

具体模型必须在统一受控评测集上比较；更换模型只产生新的 `model_name`、`model_version` 和校准版本，不改变 Candidate、Event 或 Query 契约。

### 17.5 Entity Resolver
...
- 根据投影变化计算 `LOW_STOCK`、`OBJECT_MOVED`、`INTENT_DETECTED`、`CONFIDENCE_CONFLICT` 等信号。
- 去重、冷却、优先级排序和过期。
- 选择最近的可交互设备作为默认播报端。
...
信号只表示“有值得处理的状态变化”，具体动作仍由获得授权的 Agent 完成。
...
手机是配对、信任和管理入口，核心页面包括：

1. **设备**：眼镜和戒指状态、固件、权限、最近同步。
2. **运行模式**：家庭、外出、工作、暂停、全局关闭。
3. **禁采空间**：家庭位置、Wi-Fi、手动区域。
4. **记忆**：对象、时间线、耗材、偏好和任务。
5. **纠正**：候选冲突和用户主动纠错。
6. **隐私**：近窗遗忘、单条删除、证据状态。
7. **审计**：设备何时记录、策略为何允许或阻断。

首个 PoC 可用内部 Web 控制台替代完整手机 UI，但协议和信息架构保持一致。
...
- 图片上传授权和加密。
- Vision Extractor。
- 主动语音 ASR 与 Intent/Preference Extractor。
- ObservationCandidate。
- 实体归并。
- 置信度与冲突。
- 证据删除。
...
RV101 真实采集一张图片
  → 上传到单一后端
  → 一个多模态模型抽取对象与语义位置
  → 写入 ObservationCandidate 和 MemoryEvent
...
- `Evidence → Candidate → Event → Projection` 最小数据链。
...
- 完整 CXR-L 中继；仅在 RV101 本机路径失败时启用最小替代方案。
...
| B：AI 与记忆后端 | 后端/算法负责人 | 冻结三个 JSON 契约；搭建单体 API、数据库和模型调用 | Candidate、Event、Projection、Query、删除 | 可启动服务、数据库、API 与测试 |
...
1. `CaptureEnvelope`：设备、会话、时间、触发方式、策略版本和加密媒体。
2. `ObservationCandidate`：对象类别、语义位置、时间、置信度、模型版本。
3. `FindObjectResponse`：对象、最佳位置、观察时间、置信度、来源事件。

数据库首版只保留 `evidence`、`observation_candidates`、`memory_events`、`state_projections` 和必要的 `capture_attempts`。活动期间不拆微服务，不引入 Kafka，不做通用插件系统。
...
| 风险 | 控制与降级 |
|---|---|
| RV101 后台相机或佩戴广播与文档预期不一致 | Phase 0 真机能力矩阵；可见 Activity/前台服务路径；必要时切换 CXR-L 中继或显式采集 |
...
| 断网或重试产生重复事实 | SourceEvent 与 API 幂等键、事务 Outbox、消费者去重 |
...
8. 实现策略、证据缓冲和删除链。
9. 接入 Vision/ASR，输出 ObservationCandidate。
...
1. **Reality Edge**：运行在眼镜、手机、戒指桥接器和未来设备上的会话、策略、采集和证据层。
2. **Memory Platform**：将多模态证据转化为实体事件、当前状态和可检索记忆的中台。
3. **Agent Interface**：让个人 Agent、家庭设备和未来机器人在权限范围内查询记忆、订阅信号并提供主动服务。

当前 Rokid Glass App 是 Reality Edge 的第一个实现。它既能完成眼镜上的现实采集，也为后续硬件验证统一的 Device Adapter 契约。产品先通过真机采集 PoC 证明设备能力和信任链，再逐步接入 AI、记忆主线和 Agent 服务，最终形成可被多硬件复用的现实世界记忆基础设施。
