...
> 设备投递：[设备与云端通信](05-Device-Cloud-Communication.md)
> 当前状态：Agent 权限边界可执行；提醒决策策略和下行业务载荷待专项 review
...
Signal 也不是已经投递到眼镜的提醒。Agent 和提醒策略还需要完成授权、价值、冷却、
去重、时机和终端判断，再生成通用设备消息。设备消息的业务载荷目前是草案。
...
这里的“决定”是当前职责建议，不是已冻结算法。后续 review 需要明确哪些高确定性
提醒由规则直接决定，哪些歧义情况交给 Agent，以及 Agent 输出如何被模板、长度、
过期时间和隐私策略约束。
...
6. 实现一个 Signal 订阅和提醒。
7. 使用通用设备消息投递一个测试提醒并回传回执；业务字段仍保持 draft。
8. 实现 Grant 撤销和缓存失效。
9. 最后再扩展更多 Agent、第三方账号和主动服务。
...
- 同一提醒重复拉取不会在眼镜重复展示或播报。
- 设备回执只表达投递结果，不直接修改记忆事实。
