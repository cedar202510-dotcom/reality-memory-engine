...
**已实现（`video.process`）**：视频不新建平行流水线，而是拆成系统已有的两种输入——

```
视频 ──┬─ 音轨   → ASR → AudioAsset（挂源视频）
       └─ 关键帧 → 子 EvidenceItem(media_kind=image) → 既有帧流水线
```

关键帧是等间隔采样的一等证据，靠 `evidence_items.parent_evidence_item_id`
和 `offset_seconds` 指回源视频并保留时间轴位置（后者是喜好度「画面停留时长」的来源）。
两路都不新建表，caption / CLIP / 观察抽取 / 候选门全部复用。

执行顺序是 **先音轨后画面**，因为两个方向的跨模态补全都要用到对方：

- 语音 → 画面（命名）：VLM 看着一小块炸物只能写「黄色颗粒状食物」，而同期语音是
  「这个鸡米花已经软了」。转写作为 `audio_context` 进入 caption/extract 提示词，
  画面里那团东西才能落到「鸡米花」这个名字上。
- 画面 → 语音（指代消解）：转写里满是「这个一般般」，画面物体列表作为
  `visual_context` 进入语音抽取，`object_text` 才不会抽成「这个」。

两个方向合起来的意义不只是描述更准，而是**跨模态能不能指向同一个实体**：
只要两侧的 `object_text` 对不上，实体解析就会把它们当成两个物体，
喜好度那边永远合不出「你觉得鸡米花软了不好吃」。
...
### 10.3 喜好度聚合（`GET /v1/memory/insights/preferences`）

上面两节产出的是**单条**偏好陈述。喜好度把同一实体上的所有证据聚成一个
0~100 的分数，四路证据不平权，因为它们回答的不是同一个问题：

| 通道 | 来源 | 取值 | 权重 |
|---|---|---|---|
| `verbal` 口头评价 | `PREFERENCE_STATED` 的 `sentiment` × `intensity` | −1 ~ 1 | 1.0 |
| `intent` 行动意图 | `TASK_STATED` 的 `intent_kind`（PURCHASE/REPEAT/AVOID） | −1 ~ 1 | 0.7 |
| `behavior` 实际使用 | `USED` / `CONSUMED` 观察条数 | 0 ~ 1 | 0.35 |
| `attention` 画面停留 | 出现帧数；视频来源换算成停留秒数 | 0 ~ 1 | 0.25 |

后两路**只能往正向拉**。从「他盯着看了 30 秒」推不出任何负面结论；如果允许它们取负，
一个只被拍到一次的物体就会因为「注意力少」被打成不喜欢——那是把缺少证据
错当成了负面证据。同理，`confidence` 只统计 verbal/intent 的证据量：
一个只被看见过、从没被评价过的物体，`attention` 再高也只报「证据不足」，不给结论。

两条纪律：

- **只有过了候选门的事件参与打分。** ASR 错听（实测「自己好戏也不老实」这类）
  会先落成 PENDING 候选等人确认；让候选直接改画像等于没有门。
  未决线索只体现在响应的 `pending_count` 上。
- **重复的温和评价抬高的是 confidence，不是 polarity。** 说三次「好吃」
  是「我更确定他喜欢」，不是「他更喜欢了」——所以「强烈」档的阈值
  （±38 分）要高于「好吃」标定的强度（intensity≈0.6 → 80 分）。
